...
The data-loading cell will, in order:
...
- **Undersampling correction**: `correct_undersampled_probabilities` in the
  notebook implements the closed-form correction for training on
  an undersampled majority class (Elkan, 2001; used for fraud detection in
  Dal Pozzolo, Caelen, Johnson & Bontempi, 2015).
- **Experimental differentiable calibration loss**: `soft_ece_loss` in the
  notebook is a soft-binned relaxation of ECE —
...
jupyter notebook calibration_walkthrough.ipynb
```
